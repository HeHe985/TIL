...
- 원시 자료형(Primitive type)
  ...
  - 불변(immutable)이며, 변수 간 할당시 값 복사
    - 변수 간에 서로 영향을 미치지 않음
    - 값의 일부를 직접 수정할 수 없음(```let str = 'ssafy'; str[0] = 'S'``` 처럼 할당 불가능)
...
- 참조 자료형(Reference type)
  - 데이터가 저장된 메모리의 주소가 변수에 저장되는 자료형
    - 변수 간 서로 영향을 미침
  - 가변(Mutable)이며, 변수 간 할당시 주소가 복사
    - 주소를 복사하므로, 복사본을 수정하면 원본의 값도 함께 변경될 수 있음
...
```
#### ※ 주의사항
- null의 타입은 object, undefined의 타입은 undefined이므로 타입 검사 시 주의
- return이 없는 함수나, 인자가 전달되지 않은 매개변수는 기본적으로 undefined가 할당됨
- 산술 연산 시 null은 0으로 취급되지만, undefined는 계산 불가능한 NaN 값을 만듦
### Boolean
- 참과 거짓을 나타내는 논리적인 자료형
- 조건문 또는 반복문에서 Boolean이 아닌 데이터 타입은 자동 형변환 규칙에 따라 true 또는 false로 변환됨
- 자동 형 변환
#### ![alt text](image/image1118-1.png)
# 연산자
## 연산자 종류
- 할당 연산자
- 증가 & 감소 연산자
...
- 단축 연산자 지원
#### ※ 단축 연산자: x = x + 1을 x += 1처럼 코드를 짧게 줄여주는 연산자
```javascript
let a = 0
...
- 코드의 가독성을 의해 a += 1, a -= 1과 같이 더 명시적인 표현을 권장
```javascript
let x = 3
const y = x++
console.log(x, y)
...
- 0 == false, '' == []가 true가 되는 등, 직관과 다른 암묵적 타입 변환이 일어남
- 내용이 같아도 다른 객체이면 false, [1] == [1]은 false
```javascript
...
```
- 동작 원리
#### ![alt text](image/image1118-2.png)
### for ... in
- 객체의 열거 가능한(ecumerable) 속성(property)의 키에 대해 반속
```javascript
<!-- loops-and-iteration.html -->
...
```
### 배열 반복과 for ... in
- 객체의 관점에서 보면, 배열의 인덱스도 정수형태의 이름을 가진 열거 가능한 속성
- for ... in은 정수가 아닌 이름과 속성을 포함하여 열거 가능한 모든 속성을 반환
- 내부적으로 for ... in은 배열의 반복자가 아닌 속성 열거를 사용하기 때문에 특정 순서에 따라 인덱스를 반환하는 것을 보장할 수 없음
- 따라서 for ... in은 인덱스의 순서가 중요한 배열에서는 사용하지 안흠
- 배열에서는 for 문, for ... of를 사용
- 객체 관점에서 인덱스는 정수 이름을 가진 속성이기때문에 인덱스가 출력됨(순서 보장 X)
```javascript
const arr = ['a', 'b', 'c']
...
```
#### ※ 주의 사항
- for ... in은 배열의 값이 아닌 인덱스를, 숫자가 아닌 문자열로 반환하니 주의
...
#### ![alt text](image/image1118-3.png)
# 함수
- Fucntion: 참조 자료형에 속하며 모든 함수는 Function object
## 함수 정의
### 함수 구조
```javascript
function name ([param[, param, [..., pram]]]) {
  statements
  return value
}
```
...
- 표현식(function expression)
  - 호이스팅되지 않음
    - 변수 선언만 호이스팅되고 함수 할당은 실행 시점에 이루어임
...
- NaN은 숫자가 아님이라는 뜻이지만, 타입을 확인하면 number가 반환
## null & undefined
### 값이 없음에 대한 표현이 null과 undefined 2가지인 이유
1. 역사적 맥락
   - JavaScript가 처음 만들어질 때, null은 객체가 없음을 나타내기 위해 도입
   - undefined는 나중에 추가되어 값이 할당되지 않음을 나타내게 됨
2. typeof null의 결과가 object인 이유(역사적 배경)
