...
### 비동기 실생활 예시
- 메일 전송
  - Gmail에서 메일 전송을 누르면, 받은 편지함으로 화면이 전환
  - 실제로 메일 전송 작업이 완료된 후에 화면이 전환되는 것이 아니고, 병렬적으로 별도로 처리됨
  - 메일 전송 작업이 진행되는 중에도 사용자는 다른 작업을 진행할 수 있음
- 브라우저
  - 브라우저는 웹 페이지를 먼저 처리되는 요소부터 그려나가며 오래 걸리는 것들은 별도로 처리가 완료되는 대로 병렬적으로 진행
### 비동기 코드 예시
...
- mulit-thread라면 업무를 수행할 수 있는 주체가 여러개라는 것을 의미
## JavaScript Runtime
...
- Event Loop
  - Call Stack이 비어있는지 계속해서 확인하면서 비는 순간 Task Queue에서 가장 오래된 작업을 콜 스택으로 보내는 역할
### 런타임의 시각적 표현
- 코드가 실행되는 과정
```javascript
console.log('Hi')                 // ①
...
console.log('Bye')                // ③
```
#### ![alt text](image/image1124-1.png)
- A. ① 함수가 호출되면서 Call Stack에 쌓임
- B. ① 함수는 비동기 함수가 아니기 때문에 곧바로 처리
- C. 결과에 출력
#### ![alt text](image/image1124-2.png)
- A. ② 함수가 호출되면서 Call Stack에 쌓임
- B. ② 함수는 비동기 작업이므로 직접 처리하지 않고 Web API로 보냄
#### ![alt text](image/image1124-3.png)
- A. 브라우저가 별도로 ② 함수 작업을 처리
- B. ③ 함수가 호출되면서 Call Stack에 쌓임
- C. ③ 함수는 비동기 함수가 아니기 때문에 곧바로 처리
- D. 결과에 출력
#### ![alt text](image/image1124-4.png)
- A. 비동기 함수의 작업이 끝나면, Task Queue로 보내서 콜백 함수들이 순서대로 작업이 처리될 수 있도록 함
#### ![alt text](image/image1124-5.png)
- A. Event Loop가 콜스택이 비어있는지 계속해서 확인
- B. Call Stack이 비어있는 것을 확인한 후, Task Queue에서 가장 오래된 작업을 Call Stack으로 보냄
#### ![alt text](image/image1124-6.png)
- A. ② 함수가 all Stack으로 이동한 후, 함수 내부가 동작
- B. 내부에 있는 ③ 코드가 스택에 다시 쌓임
- C. ③ 코드는 비동기 코드가 아니기 때문에 곧바로 처리
- D. 코드 출력
...
- 이벤트 핸들러는 코드를 막지 않음
- 요청을 보내고, 응답을 기다리는 동안 다른 작업을 처리할 수 있음
- 요즘 HTTP 요청은 XHR 이벤트 핸들러보다, Promise와 async/await를 쓰는 것이 더 현대적
## Axios
- 브라우저와 Node.js 환경에서 모두 사용할 수 있는 Promise 기반의 HTTP 클라이언트 라이브러리
- 간편하게 Ajax 통신을 할 수 있도록 도와주는 JavaScript 라이브러리
- 브라우저와 Node.js 환경 모두에서 똑같은 코드로 작동
- 데이터 변환이나 에러 처리 등이 편리해, 많은 실무 프로젝트에서 비동기 HTTP 통신을 위해 가장 많이 사용됨
### Axios 정의 및 특징
- 클라이언트 서버 사이에 HTTP 요청을 만들고 응답을 처리하는 데 사용되는 자바스크립트 라이브러리
...
#### ![alt text](image/image1124-7.png)
### Axios 설치 및 사용 
...
```
### 고양이 사진 가져오기 실습 - 심화
- 버튼을 누르면 고양이 이미지를 요청하고 요청이 처리되어 응답이 오면 응답 데이터에 있는 이미지 주소 값을 img 태그에 넣어 이미지 출력하기
...
- 개발자 입장에서 코드의 실행 순서가 불명확하다는 단접 존재하고, 이로 인해 실행 결과를 정확히 예측하며 코드를 작성하기 어려울 수 있음
## 비동기 처리 관리 방법
1. 비동기 콜백
...
- 작업의 순서와 동작을 제어하거나 결과을 처리하는 데 사용
```javascript
const asyncTask = function (callBack) {
  setTimeout(function () {
...
```
### Promise
- then 메서드를 통해 여러 비동기 작업을 연결하는 체이닝하여 순차적으로 연결
```javascript
work()
  .then((result1) => {
    // work2
...
- chaining을 활용해 cat api 실습 코드 개선하기
  - 첫번째 then 콜백함수의 반환값이 이어지는 then 콜백함수의 인자로 전달
```javascript
// 기존
.then((response) => {
  const imgUrl = response.data[0].url
...
- 예2: 요청 실패 시 다시시도와 같은 행동을 안내해야 함
## 비동기 처리 적용 사례
1. 소셜 미디어 피드
...
- 예시: 비동기 결제 처리
  ...
  - (비동기) 결제 성공/실패 응답을 받음
  - (동기) 응답이 성공일 경우, 우리 서버 데이터베이스에 주문 상태를 결제 완료로 기록
