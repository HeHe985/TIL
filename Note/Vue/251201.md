...
```
## Computed vs. Methods
### computed롸 동일한 로직을 처리할 수 있는 method
...
```
### computed롸 method 차이
- computed 속성은 의존하는 반응형 데이터를 기반으로 그 결과를 채시(cached)
...
- computed
  ...
  - 동일한 의존성을 가진 여러 곳에서 사용할 때 계싼 결과를 캐싱하여 중복 계산 방지
- method
...
- v-if는 특정 조건이 참일 때만 HTML 요소를 화면에 보여주도록 하는 Directive임
- 조건이 거짓이면, 해당 요소는 DOM(문서 구조)에서 완전히 제거되어 보이지 않게 됨
...
- 요소를 자주 보여주고 숨겨야할 경우, 렌더링 비용이 높은 v-if 보다 성능적으로 유리
### v-show 예시
...
# List Rendering
## v-for
- 소스 데이터를 기반으로 요소 또는 템플릿 블록을 반복 렌더링(Array, Object, Number, String, Iterable)
- v-form는 배열이나 객체의 데이터를 렌더링 하는 반복문 Directive
...
- key를 통해 이 항목은 이 데이터에 해당한다는 힌트를 줌으로써 변경시에도 올바른 항목만 효율적으로 업데이트할 수 있음
...
- v-if가 더 높은 우선순위를 가지므로 v-for 범위의 todo 데이터를 v-if에서 사용할 수 없음
  - Uncaught Type/errir: Cannot read properties of undefined (reading 'isComplete')
- 동일 요소에 v-for와 v-if를 함께 사용하면 안됨
  - 동일 요소에서 v-if가 v-for보다 우선순위가 더 높기 때문
  - v-if에서의 조건은 v-for 범위의 변수에 접근할 수 었음
### v-for와 v-if 해결법 2가지
1. computed 활용
...
```
### v-for와 v-if 해결법 2: '\<template>' 요소 활용
...
- watch는 데이터(ref)의 변화를 지켜보다가, 값이 바뀔 떄마다 지정된 콜백 함수를 실행하는 기능
- 새로운 값을 계싼하는 computed와 달리, watch는 데이터가 바뀔 때 특정 행동(Side Effect)을 수행하기 위해 사용됨
### watch 구조
1. 첫번째 인자(source)
...
- 버튼을 누를 때마다 count 값이 바뀌고, watch는 그 변화를 감시하고 있다가 즉시 콜백함수를 실행
```html
...
- 여러 소스를 감시할 때, 콜백의 인자(새 값, 이전 값)도 같은 순서의 배열로 전달됨
...
#### ![alt text](image/image1201-1.png)
# Lifecycle Hooks
...
## 주요 Lifecycle Hooks
- 생성 단계 / 마운트 단계 / 업데이트 단계 / 소명 단계 등 다양한 단계 존재
...
- Lifecycle Hooks는 반드기 동기적으로 작성해야 함
...
```
2. method 활용
    - computed가 불하능한 중첩된 v-for의 경우 / 매개변수가 필요한 경우
```javascript
